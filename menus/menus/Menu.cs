...
namespace menus
{
    class Menu
    {
        ...
        SpriteFont font;
        //Do I need to pass Game1?  Is there some better way to 
        //access the game's resources besides passing the object around?
        Game1 game;

        Texture2D blanksprite, menusprite;
        ...
        void DrawString(string str, bool selected, int yPos)
        {
            ...
            spriteBatch.DrawString(font, str, new Vector2((game.GraphicsDevice.Viewport.Width / 2) - 
        ...
        public void Draw(GameTime gameTime)
        {
            ...
            spriteBatch.Begin();
            spriteBatch.Draw(blanksprite, new Rectangle(0, 0, game.GraphicsDevice.Viewport.Width, 
                game.GraphicsDevice.Viewport.Height), Color.Black * backgroundTransition);

            spriteBatch.Draw(menusprite, new Rectangle((int)(game.GraphicsDevice.Viewport.Width/2 - currentWidth/2), 
                (int)(game.GraphicsDevice.Viewport.Height/2 - currentHeight/2), (int)currentWidth, (int)currentHeight), Color.White);

            spriteBatch.DrawString(font, selected.ToString(), new Vector2(300, 500), Color.White);

            if (transitionLevel > 1)
            {
                DrawString(items[0], false, 200);
                int i = 1;
                int yPos = 300;
                while (i <= listCount)
                {
